...
DefaultTransporter.prototype.request = function(opts, opt_callback) {
  ...
  // HACK Issue when inserting / updating user and using qs (aka query string), so setting json instead of qs
  if ((opts.method === 'POST' || opts.method === 'PUT') && opts.uri != 'https://accounts.google.com/o/oauth2/token') {
    opts.json = opts.qs;
    delete opts.qs;
  }
